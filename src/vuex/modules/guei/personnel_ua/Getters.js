...
const temp_moyen_fin_activite_interruption = state =>
  state.temp_moyen_fin_activite_interruption;
const delais_mise_disposition_act = state => state.delais_mise_disposition_act;
const jour_conge_disponible_acteur = state =>
  state.jour_conge_disponible_acteur;
const totalActeurEnctivite = state =>
  state.acte_personnels.filter(
    acteur_depense => acteur_depense.date_fin_contrat == null
  ).length;
const totalActeurDepense = state => state.all_acteur_depense.length;
const totalActeurNonAccredite = state =>
  state.acte_personnels.filter(
    acteur_depense =>
      acteur_depense.type_acte_id != "4" &&
      acteur_depense.date_fin_contrat == null
  ).length;
const totalActeurAccredite = state =>
  state.acte_personnels.filter(
    acteur_depense =>
      acteur_depense.type_acte_id == "4" &&
      acteur_depense.date_fin_contrat == null
...
const tauxActeurAccredite = (state, getters) =>
  parseFloat(
    (getters.totalActeurAccredite * 100) / getters.totalActeurEnctivite
  ).toFixed(2);
//const totalActeur 4
//const exercices_budgetaires = state => state.exercices_budgetaires
export const personnaliseActeurDepense = (
  state,
  getters,
  rootState,
  rootGetters
