...
const MODIFIER_EXERCICE_BUDGETAIRE = (state, objetModifie) => {
  state.exercices_budgetaires = state.exercices_budgetaires.map(
    exercice_budgetaire => {
...
const SUPPRIMER_EXERCICE_BUDGETAIRE = (state, id) => {
  state.exercices_budgetaires = state.exercices_budgetaires.filter(
    exercice_budgetaire => exercice_budgetaire.id != id
  );
};
...
const SUPPRIMER_NATURE_SECTION = (state, id) => {
  state.natures_sections = state.natures_sections.filter(
    nature_section => nature_section.id != id
  );
};
...
const MODIFIER_STRUCTURE_PROGRAMME = (state, elementModif) => {
  state.structures_programmes = state.structures_programmes.map(
    structure_programme => {
...
const SUPPRIMER_STRUCTURE_PROGRAMME = (state, id) => {
  state.structures_programmes = state.structures_programmes.filter(
    structure_programme => structure_programme.id != id
  );
};

// get alll structure administrative
const GET_STRUCTURE_ADMINISTATIVE = (
  state,
  tableau_structure_administrative
) => {
...
const MODIFIER_STRUCTURE_ADMINISTRATIVE = (state, objetModifie) => {
  state.structures_administratives = state.structures_administratives.map(
    structure_administative => {
...
const SUPPRIMER_STRUCTURE_ADMINISTRATIVE = (state, id) => {
  state.structures_administratives = state.structures_administratives.filter(
...
const MODIFIER_SERVICE_GESTIONNAIRE = (state, objetModifie) => {
  state.services_gestionnaires = state.services_gestionnaires.map(
...
const SUPPRIMER_SERVICE_GESTIONNAIRE = (state, id) => {
  state.services_gestionnaires = state.services_gestionnaires.filter(
    service_gestionnaire => {
...
const SUPPRIMER_GRANDE_NATURE = (state, id) => {
  state.grandes_natures = state.grandes_natures.filter(
    grande_nature => grande_nature.id != id
  );
};
...
const MODIFIER_STRUCTURE_GEOGRAPHIQUE = (state, objetModifie) => {
  state.structures_geographiques = state.structures_geographiques.map(
    geographie => {
...
const SUPPRIMER_STRUCTURE_GEOGRAPHIQUE = (state, id) => {
  state.structures_geographiques = state.structures_geographiques.filter(
    geographie => geographie.id != id
  );
};
...
const GET_LOCALISATION_GEOGRAPHIQUE = (
  state,
  tableau_localisation_geographique
) => {
...
const MODIFIER_LOCALISATION_GEOGRAPHIQUE = (state, objetModifie) => {
  state.localisations_geographiques = state.localisations_geographiques.map(
    geographie => {
      if (geographie.id == objetModifie.id) {
        geographie = { ...objetModifie };
      }
...
const SUPPRIMER_LOCALISATION_GEOGRAPHIQUE = (state, id) => {
  state.localisations_geographiques = state.localisations_geographiques.filter(
    geographie => geographie.id != id
  );
};
